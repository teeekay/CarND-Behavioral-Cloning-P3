...
use_Udacitydata = True #True
use_track1_fixes = True
use_track2 = True
...
checkpoint = ModelCheckpoint('model3a_hsv{epoch:02d}.h5', monitor='lr',verbose=2)
earlystop = EarlyStopping(monitor='val_loss', min_delta=0.002, patience=0, verbose=2, mode='auto')
...
for i in range(len(steers)):
    ...
    displayCV2(img)
    cv2.imwrite("./model3a_hsv"+str(i)+".png", img)
